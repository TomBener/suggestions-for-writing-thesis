...
再补充一下，EndNote、NoteExpress、Mendeley 这类文献管理软件大同小异，如果你不喜欢 EndNote，或者因为某些原因无法正常使用 EndNote，尝试使用其他的软件也是没有问题的，毕竟有些还是完全免费的，比如 Mendeley、Zotero。
...
![](https://i.loli.net/2019/10/04/SCGfR5pJ6aEAs83.png)**4.  添加引文数据至 EndNote 数据库**
...
![](https://i.loli.net/2019/10/04/pxSX9vnudmoHDTC.png)**5.  在 Word 中引用文献**
...
- 在 EndNote 中选择需要引用的文献（按住 `shift` 键可以选择多个）
...
需要注意的是，直接下载安装的 `Chinese Standard GB/T7714 (numeric)` 有一些小问题，比如括号只有一半、作者姓名大小写等问题，需要自行修改，上图中的最终效果是我修改后的结果。依次点击 `Edit -> Output Styles -> Edit "Chinese Std GBT7714 (numeric)"`，在弹出的窗口中（如下图所示），将 Journal Article 修改为如下格式：
